...
public class ModNetworking {
    ...
    public static void registerServer() {
        ServerPlayNetworking.registerGlobalReceiver(JETPACK_ACTION, (server, player, handler, buf, responseSender) -> {
            ...
            server.execute(() -> handleJetpackAction(player, actionOrdinal));
        });
    ...
    private static void handleJetpackAction(net.minecraft.server.network.ServerPlayerEntity player, int actionOrdinal) {
        if (actionOrdinal < 0 || actionOrdinal >= JetpackAction.values().length) {
            return;
        }

        ItemStack stack = JetpackFlightHandler.findEquippedJetpack(player);
